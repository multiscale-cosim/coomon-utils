...
class ApplicationManager(multiprocessing.Process):
    ...
    def __init__(self, log_settings, configurations_manager, actions,
        ...
        self.__applicaiton_manager_response_queue = am_response_queue
        ...
        self.__logger.debug("Application Manager is initilized.")
    ...
    def run(self):
        ...
        try:
            ...
            self.__applicaiton_manager_response_queue.put(Response.ERROR)
        ...
        self.__logger.info(f'Applciation <{application}> finished properly.')
        # stop resource monitoring
        if self.__is_monitoring_enabled:
            resource_usage_monitor.keep_monitoring = False
            resoruces_usage_by_popen_process = resource_usage_monitor.\
                get_resource_usage_stats(return_code)
            self.__logger.debug(f"Resource Usage stats: "
                                f"{resoruces_usage_by_popen_process.items()}")
            ...
            self.__db_manager_file.write(metrics_file, resource_usage_monitor.
                                         get_resource_usage_stats(return_code))
        self.__applicaiton_manager_response_queue.put(Response.OK)
